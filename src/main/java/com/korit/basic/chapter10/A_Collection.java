...
/*
    === 자바 컬렌션 프레임워크(Framework) ===
    : frame(틀, 구조) + work(일하다)
    - 제공받은 일정한 요소와 틀, 규약을 가지고 무안가를 만드는 일

    1. 컬렉션 플레임워크(Collection Framework)?
    : 데이터를 저장하고 처리하는 자료구조의 표준화돤 설계
    - 다양한 자요구조를 제공, 데어티의 효율적인 관리 지원
    ...
    2. 컬렉션 프레임워크의 주요 '인터페이스'와 특성
        ...
        2) List
        - 데이터의 순서 유지(Index로 접근 가능)
        - 중복 허용
        - 주요 구현체 : ArrayList, LinkedList, Vector 등

        3) Set
        - 데이터의 순서 유지 X (특정 구현체는 정렬 또는 순서 제공 가능*)
        - 중복 허용 X
