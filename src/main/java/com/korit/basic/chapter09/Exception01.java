...
/*
    ...
    2. 자바 예외 클래스 구조
        ...
        - 모든 예외 클래스는 해당 클래스를 상속 받는다
        >> 예외와 관련되니 메시지, 처리 방법에 대한 예시 정보를 포함
 */
...
/*
        1) Exception (예외)
        : 프로그렘이 처리할 수 있는 예외들을 정의
        >> 에외 처리를 통해 개발자가 처리할 수 있는 문제들을 정의


            1-1) Checked Exception
            : 컴파일 시점에 체크되는 예외
            - 컴파일러는 해당 예외가 처리되어 있지 않을 경우 오류 발생
            - 개발자가 직접 반드시 처리
            ...
            - 런타임 : 컴파일된 프로그램이 실행되는 단계, 프로그램이 실제로 메모리에 로드되고 CPU에서 실행됨
                컴파일 시점에서 자히지 앟은 논리적 오류나 런타임 예외 발생 가능성이 존재
    ...
    3. 자바 예외 처리 방법
    : try-catch 블록, throwable 키워드, trow 키워드, 사용자 정의 예외
    ...
    - catch 블록 : 특정 에외 타입이 발생했을 때 실행될 코드를 정의
...
//	      at chapter09.Exception01.main(Exception01.java:59)
...
//            try 블록 내에서 발생하는 예외를 () 소괄호 내에서 매개변수로 전달 받아 {} 중괄호 실행문에서 처리
...
//            null을 계산할 경우 예외 발생 : NullPointerException
            ...
            System.out.println("수학적 공식의 에러 발생");
            System.out.println(e.getMessage());
        } catch (ArrayIndexOutOfBoundsException e) {
            System.out.println("배열 인덱스 초과 오류 발생");
            System.out.println(e.getMessage());
        } catch (NullPointerException e) {
            System.out.println("null 계산할 경우 오류 발생");
            System.out.println(e.getMessage());
        } catch (Exception e) {
            System.out.println("초과 오류 발생" + e.getMessage());
        } finally {
//            cf) 예외 발생 여부와 상관없이 항상 실행
...
//        === throw 키워드 ===
//        : 프로그램 실행중에 특정 조건에서 예외를 직접 발생시킬 때 사용
//        > 주로 조건문과 함꼐 사용
//        사용법
//        : throw new ExceptionTYpe("예외 메시지");
//        : 인자로 전달되는 문자열 예외 메시지는 getMessage(); 메서드를 통해 출력
        int age = -10;
        try {
            if (age < 0) {
