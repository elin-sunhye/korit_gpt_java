...
/*
    === Lombok 라이브러리 ===
    : 보일러플레이트 코드를 줄여주는 라이브러이
    - 어노테이션을 사용하여 코드를 간결하게 만들어 줌

    cf) 보일러플레이트 코드 : 프로그래밍에서 반복되는 작업이나 패턴에 대한 일종의 표준화된 코드

    1. @Getter / @Setter 어노테이션
    : 필드에 대한 @Getter / @Setter 메서드를 자동 생성

    2. @ToString 어노테이션
    : 클래스의 toString() 메서드 자동 생성
    - sout에 객체를 출력하는 경우 : 클래스명(필드명1=필드값1, 필드명2=필드값2...)

    3. 생성자 어노테이션
    cf) 매개변수-parameter(params), 인자(인수)-argument(args)
        1) @NoArgsConstructor : 기본 생성자 (빈 생성자)
        2) @RequiredArgsConstructor : 필수 필드 생성자 (final 초기식)
        3) @AllArgsConstructor : 전체 필드 생성자 (필드 생성 순서대로 인자 전달)

    4. @Data 어노테이션
    : @Setter, @Getter, @ToString, @RequiredArgsConstructor, @EqualsAndHashCode 어노테이션을 한번에 적용하는 어노테이션

    5.@Builder 어노테이션
    : 복잡한 객체를 생성할 때 사용하는 빌더 패턴을 자동 생성
    - @Builder 어노테이션을 사용하는 빌더 패턴의 경우 필드 값을 모두 메서드 체이닝으로 가져옴
    - 클래스명.builder().build();
