...
/*
    ...
    : Arrays.sort()
    - 오름차순(가, 나, 다 / 1, 2, 3) 정렬만 지원
    cf) 내림차순(하, 파, 타 / 10, 9, 8) 정렬은 Comparator 클래스 사용

    2. 리스트(List 클래스) 정렬
    : Collections.sort() 또는 List 클래스의 sort() 메서드 사용
 */
public class A_Sort {
    public static void main(String[] args) {
//        1. 배열(Arrays 클래스) 정렬
//          1) 오름차순(가, 나, 다 / 1, 2, 3) 정렬
        int[] numbers = {5, 3, 2, 8, 1};
        Arrays.sort(numbers); // 오름차순(가, 나, 다 / 1, 2, 3) 정렬
        System.out.println(Arrays.toString(numbers));

//          2) 내림차순(하, 파, 타 / 10, 9, 8) 정렬 - Comparator 클래스 내의 reverseOrder() 정적 메서드를 사용
//          Collections.reverseOrder() : Comparator<T>를 반환하는 메서드 : 객체간의 비교를 실시
...
//        2. 리스트(List 클래스) 정렬
//          1) 오름차순(가, 나, 다 / 1, 2, 3) 정렬
        List<Integer> numbers3 = new ArrayList<>(List.of(5, 3, 9, 7, 6));
        Collections.sort(numbers3);
        System.out.println(numbers3);

//          2) 내림차순(하, 파, 타 / 10, 9, 8) 정렬
...
//        : List 인터페이스의 메서드
//        - 리스트를 특정 기준에 딸 정렬
//        - 정렬 기준을 결정하는 Comparator(두 객체를 비ㅛ하여 정렬 순서를 결정)를 인수로 받음
//
