...
/*
    ...
    >> 모듈의 독립성을 결합도(Coupling) & 응집도(Cohesion)의 기준 단계로 측정
    ...
    >> 낮은 결합도 : 프로그램를 유지보수하기 쉽게 만들고, 새로운 기능을 추가하거나 기존 코드를 수정할 때 오류 발생 가능성을 낮춤

    2. 응집도(Cohesion) - 높은 응집도
    : 하나의 클래스나 모듈 안에서 관련된 작업들끼리 얼마나 잘 모여 있는지를 나타냄
    : 한 모듈 내의 구성요소들 간의 연관 정도

    - 높은 응집도 : 하나의 클래스가 관련된 일들만 하고, 헤당 일이 잘 명시
    - 낮은 응집도 : 하나의 클래스가 너무 많은 일을하거나, 관련 없는 일을 하는 것
    >> 높은 응집도 : 프로그램에 대한 이해, 유지보수 용이
    ...
    == 낮은 결합도와 높은 응집도 구현 ==
    : 클래스 간의 의존성을 줄이고, 변경에 강한 구조로 작성

        1) 인터페이스와 다형성 활용
            : 구현 클래스에 의존하지 않고, 인터페이스를 통해 서로 의존하게 구현 (DIP)
            >> 구현이 변경되거나 확장되어도 인터페이스만 맞추면 동작
 */
...
//    : 클래스가 직접 갹채를 생성하지 않고, 외부에서 객체를 주입받도록 설계
//    - 객체 간의 강한 결합을 줄이고, 변경에 유연
//    CreditCardPayment creditCardPayment = new CreditCardPayment(); -> XXX
...
//        creditCardPayment.processPayment(); -> XXX
    }
}
...
class User {
    //    2-2) 캡슐화
//        : 필드를 private 설정, getter와 setter 를 통해 접근
...
//        저장 로직 구현
        System.out.println("시용자 정보가 데이터 베이스에 저장되었습니다.");
    }
}
...
/*
    ...
    높은 응집도 : 단일 책임 원칙 준수, 캡슐화, 모듈화(메서드 분리) 둥
 */