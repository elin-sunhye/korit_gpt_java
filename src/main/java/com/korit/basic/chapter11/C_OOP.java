...
/*
    ...
    2. OCP(개방-폐쇄 원칙)
    - 클래스는 확장에 열려 있고, 수정에는 닫혀 있어야한다.

    3. LSP(리스코프 치환 원칙)
    - 자식 클래스는 부모 클래스를 대체 할 수 있어야한다.

    4. ISP(인터페이스 분리 원칙)
    - 클라이언트가 자신이 사용하지 않는 메서드는 의존하지 않아야한다.
...
//    1.  SRP(단일 책임 원칙) : 각 클래스는 하나의 책임만 가짐
class Order {
...
class CreditCardPaymentProcessor implements PaymentProcessor {
    ...
    public void processPayment(Order order) {
        System.out.println("신용카드 결제 : " + order.getOrderId());
    }
}
...
class PaypalPaymentProcessor implements PaymentProcessor {
    ...
    @Override
    public void processPayment(Order order) {
        System.out.println("페이팔 결제 : " + order.getOrderId());
    }
}

// ISP : 클라이언트가 불필요한 메소드에 의존 X
...
//    CreditCardPaymentProcessor, PaypalPaymentProcessor (저수준 모듈)
    private final PaymentProcessor paymentProcessor;
    private final GenerateReport generateReport;

    //    OrderService 생성자 : 결제 처리기와 보고서 처리기를 주입받음
    public OrderService(PaymentProcessor paymentProcessor, GenerateReport generateReport) {
        this.paymentProcessor = paymentProcessor; // 결체 처리기 초기화
